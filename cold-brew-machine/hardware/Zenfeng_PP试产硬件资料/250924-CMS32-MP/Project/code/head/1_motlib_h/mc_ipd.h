...
typedef struct
{
	u8			Correct_Flag;			//脉冲正确识别标志
	u8			InjectRound;			//脉冲注入轮次标识，如0代码第一轮脉冲注入
	u8			InjectAngleCounter;		//脉冲序号
	u16			InjectTimeCounter;		//一轮脉冲需要的总时间
	u16			InjectAngleIndex;		//最大id值标号
	u16			InjectAngleIndex2;		//最大id值标号
	s32			InjectAngle;			//脉冲注入角度值
	s32			InjectMaxAcquiredValue;
	u16			InjectTime;
	s32			InjectCurrent;
	s32			InjectCurrent_Base;
}Pulse_Component_t;
...
/*****************************************************************************/
extern s32 		FOC_IPD_PulseSine(Pulse_Component_t	*p);
extern void 	FOC_IPD_Init(Pulse_Component_t *p);
extern u32 		MC_IPD_PulseSquare(Pulse_Component_t *p);
