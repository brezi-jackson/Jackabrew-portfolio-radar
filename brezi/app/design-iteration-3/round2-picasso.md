# Brezi iOS App — Visual Coffee Layer & Marketplace v2.0
**Author:** Picasso (Product Design) | March 20, 2026 | Iteration 3 — Round 2
**For:** Jackson (Product) · Jensen (Engineering) · Elon (Growth/Copy)
**Status:** DRAFT. Complete screen-by-screen redesign incorporating all Round 1 feedback.

---

## 0. What Changed Since Round 1

Round 1 built the architecture. Round 2 gives it a soul. Here's what moved:

| Change | Why |
|--------|-----|
| **Why Today screen designed** | Jackson's locked flow requires it. It's the emotional bridge and palette transition vessel. Was missing entirely. |
| **Corvus bean corrected** | Was Ethiopia Guji Barrel Aged (wrong). Now Sadayana Winey from West Java (correct per Jackson's locked data). |
| **"Your History With This Bean" designed** | Jackson's locked feature. Retention driver. Was missing entirely. |
| **Bean card context modes** | `.selection` / `.marketplace` / `.detail` — separates brew intent from purchase intent. |
| **Image container → 280pt** | Per-roaster `imageBgTint` for visual personality. Stitch's bear breathes. |
| **Clock arc killed** | Redundant with timing bar. Replaced by bold cutoff time label. |
| **General Profile as equal card** | Moved out from behind divider. Same visual weight as bean cards. |
| **Recommendation tag → "TODAY'S PICK"** | Sounds like a barista, not a recommendation engine. |
| **Card 3 bridge copy** | Body-state reflections per profile, connecting health read to action. |
| **Subscribe & Save fully designed** | CTA priority flips after 3+ brews. Subscription management in Settings. |
| **Regent gets 4th tasting note** | Lime was missing. Added per Jackson's locked data. |
...
struct WhyTodayTransition {
    ...
    let transitionDuration: TimeInterval = 1.8   // total bg transition
    let transitionDelay: TimeInterval = 0.4      // wait for first line to start rendering
    let transitionCurve: Animation = .easeInOut(duration: 1.8)

    // The text renders character-by-character
    let typewriterSpeed: TimeInterval = 0.03     // seconds per character
    let lineDelay: TimeInterval = 0.3            // pause between lines
}
```
...
│                                     │
│                                     │
│  Something gentle.                  │  LINE 3: the coffee direction
│  Let these do the talking.          │  Regular, 18px, animated #888888 → #8C8279
...
│  └─────────────────────────────────┘│  fadeIn + translateY(12→0), delay: 0.3s
...
### Copy — Per Profile (exact strings)
...
2. **The interpretation** — what it means for the body (one sentence)
...
- **First visit of the day:** Full typewriter render + palette transition. ~4 seconds total.
...
│  ┌──────┐ ┌──────┐ ┌──────┐ ┌────┐ │
│  │  🍫  │ │  🌸  │ │  🍓  │ │ 🍋 ││  TASTING NOTE ICONS
│  │Cocoa │ │Jasmi-│ │Straw-│ │Lime││  See Section 8 for full spec
│  │      │ │ne    │ │berry │ │    ││  top padding: 12pt
│  └──────┘ └──────┘ └──────┘ └────┘ │
│                                     │
...
struct BeanCardSpec {
    ...
    // Image container
    let imageHeight: CGFloat = 280         // CHANGED from 220 — roaster art breathes
...
#### Bean 2: Corvus Coffee Roasters — Sadayana Winey (CORRECTED)
...
let corvus_sadayana = Bean(
    ...
    roastLevel: .medium,           // CORRECTED — was .light
    ...
    bestForProfiles: ["deep_work", "voltage"],  // CORRECTED — was just ["voltage"]
    price: nil,                    // TBD
    shopifyURL: nil,               // TBD
    shopifyProductID: nil,
...
let regent_guji = Bean(
    ...
    tastingNotes: [
        ...
        TastingNote(icon: "icon_lime", label: "Lime")         // ADDED — was missing in R1
    ],
...
struct Bean: Identifiable, Codable {
    ...
    let showEmotionalTagOnCard: Bool    // NEW — true for Stitch
    ...
    let imageBgTint: String             // NEW — hex color for image container bg
...
// Profile → Bean mapping (CORRECTED — current catalog)
// tide_pool   → Regent Ethiopia Guji Hambela
// first_light → Regent Ethiopia Guji Hambela
// deep_work   → Corvus Sadayana Winey, Stitch Dark Field    ← CHANGED: Corvus added
// voltage     → Corvus Sadayana Winey, Stitch Dark Field    ← CHANGED: Corvus corrected
...
- Marketplace context:
  ...
  - Tap "NOTIFY ME" → stores notification preference in UserDefaults, shows toast: `"We'll let you know."`

```swift
let bean_priceTBD_selection    = "Coming soon"
let bean_priceTBD_marketplace  = "PRICE COMING SOON"
let bean_notifyMe              = "NOTIFY ME"
let bean_notifyMe_toast        = "We'll let you know."
```
...
│                                     │  top padding: 20pt
│  Clean start. These match           │  Regular, 15px, #8C8279
│  the morning.                       │  line-height: 1.5
...
let beanSelection_headerCopy: [String: String] = [
    ...
    "first_light": "Clean start. These match the morning.",
...
The general profile option is now an **equal card** in the same scroll as the bean cards. Same component structure (`#FFFFFF` bg, 20pt radius, same shadow), same height feel. It is the last card before the "browse" divider.
...
For horizontal scroll in "Browse All Beans". No sliders value labels. No CTA — tap goes to BeanDetailView.
...
**Cart icon (🛒) in nav bar:** Pushes to `MarketplaceView`.
...
│  ┌─────────────────────────────────┐│
│  │                                 ││  SERVING INFOGRAPHIC
│  │     [See Section 7 — full       ││  ServingInfoCard.swift
│  │      infographic card]          ││
│  │                                 ││
...
let beanAdjustments: [String: [String: BeanAdjustment]] = [
    ...
    "corvus_sadayana_winey": [                    // CORRECTED ID
...
│  │  │  You've brewed this 5×.   │  ││  See Section 9 for full spec
...
│  ┌─ SERVING GUIDE ─────────────────┐│
│  │                                 ││  INFOGRAPHIC CARD
│  │     [See Section 7]             ││  ServingInfoCard.swift
│  │                                 ││
│  └─────────────────────────────────┘│
│                                     │
│  ┌─────────────────────────────────┐│  BREW + PURCHASE
...
The app is saying: *"You clearly love this. Let us keep it coming."*
...
Opens Shopify subscription page in SFSafariViewController. Button hidden when `bean.price == nil` or `bean.shopifyURL == nil`.
...
func subscribeSavingsText(price: Double) -> String {
    let savings = price * 0.15
    let subscriptionPrice = price - savings
...
struct FlavorSliderSpec {
    ...
    let sweetnessGradientDark: [Color] = [
        Color(hex: "#C8BFB0"),   // CHANGED — brighter than #E5DDD3
        Color(hex: "#F5E6C8"),
        Color(hex: "#F0D4A0"),
        Color(hex: "#E8B860"),
        Color(hex: "#D4A96A")
    ...
    let acidityGradientDark: [Color] = [
        Color(hex: "#C8CCA0"),   // CHANGED — brighter for dark card contrast
        Color(hex: "#D4D8A0"),
        Color(hex: "#C8D070"),
        Color(hex: "#B8C850"),
        Color(hex: "#88B830")
...
enum TastingNoteIcon: String, CaseIterable {
    ...
    // Fruit family
    case raspberry = "icon_raspberry"               // raspberry with drupelets — NEW
    ...
    case lime = "icon_lime"                         // lime wedge — NEW for Regent
...
### Tasting Note Mapping — 3 Real Beans (CORRECTED)
...
let beanTastingNotes: [String: [TastingNoteDisplay]] = [
    ...
    "corvus_sadayana_winey": [                                // CORRECTED ID + notes
        TastingNoteDisplay(icon: .raspberry, label: "Raspberry Yogurt"),
    ...
    "regent_ethiopia_guji_hambela": [
        ...
        TastingNoteDisplay(icon: .lime, label: "Lime")        // ADDED — was missing in R1
    ]
]
```
...
**Lead icon treatment:** First icon in each bean's tasting note array gets a subtle tinted background (`#D4A96A` at 20% opacity) to establish visual hierarchy. One glance should say: *"This is a raspberry-forward coffee."*
...
No clock arc. Just the time and a human reason.
...
2. **Navigation bar cart icon (🛒)** — on coffee-side screens
3. **Dashboard** — "BROWSE BEANS" button (now above "VIEW BREW HISTORY")
...
### All Copy — Marketplace (exact strings)
...
On coffee-side screens, nav bar right slot shows cart icon:
...
AppRoot
 └─ MainFlow
     ...
     │   │   ├─ "CHOOSE YOUR BEAN" → **WhyTodayView.swift** [NEW — Screen 2]
     │   │   │   └─ "SEE YOUR BEANS" → **BeanSelectionView.swift** [NEW — Screen 3]
     │   │   │       ├─ "BREW WITH THIS" → **BrewSettingsView.swift** [NEW — Screen 4]
     ...
     └─ SettingsView.swift
         ├─ **"Bean Shop"** → MarketplaceView [NEW]
         └─ **"Manage Subscriptions"** → SFSafariViewController [NEW]
```
...
|---|---|---|---|---|---|---|
| **Why Today** | Background color | crossfade (#0F0F0F → #FAF6F0) | 1.8s | easeInOut | 0.4s | Screen appear (first per day) |
| **Why Today** | Text color | crossfade (#FFFFFF → #1A1714) | 1.8s | easeInOut | 0.4s | Synced with bg |
| **Why Today** | Message text | typewriter (char by char) | ~3s | linear 0.03s/char | 0.0s | Screen appear (first per day) |
| **Why Today** | Direction text | typewriter | ~2s | linear 0.03s/char | 0.3s after msg | After message completes |
| **Why Today** | CTA button | fadeIn + translateY(12→0) | 0.3s | easeOut | 0.3s | After direction completes |
...
## 17. Decisions — Iteration 3, Round 2

1. **Why Today is the palette transition vessel.** The screen opens dark, renders a typewriter message about the user's body state, and the background crossfades to warm cream as the text appears. This is the hero brand moment — not a CSS transition, not a navigation push. The app exhales.
2. **Corvus bean is corrected.** Sadayana Winey from West Java, Indonesia. Raspberry yogurt / cherry syrup / dark chocolate. Medium roast. Best for Deep Work and Voltage. All data, tasting notes, flavor profile, and recommendation mapping updated.
3. **"Your History With This Bean" is designed.** Appears after 3+ brews. Shows count, best recovery day, mini chart of recent brews, and an insight line. Lives in BeanDetailView's BREW PAIRING section and as a compact card on marketplace bean cards.
4. **Bean cards have three context modes.** `.selection` shows "BREW WITH THIS" only (no purchase CTA). `.marketplace` shows full purchase UI. `.detail` shows both brew and purchase CTAs with spatial separation.
5. **General Profile is an equal card.** Same visual weight as bean cards — 280pt image area, same card component, same green CTA button. Not behind a divider. Not framed as absence. Positioned as last card in the recommended scroll.
6. **Recommendation tag is "TODAY'S PICK."** Not "RECOMMENDED FOR YOU." Sounds like a barista, not a recommendation engine.
7. **Image containers are 280pt with per-roaster imageBgTint.** Stitch gets warm charcoal (#2E2022), Corvus gets deep gold (#2A2418), Regent gets neutral cream (#F5F0E8). The roaster's personality bleeds through the image treatment.
8. **Lead tasting note icon gets accent background.** First icon in the array gets a subtle tinted bg (#D4A96A at 20%) to establish visual hierarchy.
9. **Clock arc is dead. Timing bar lives.** Bold cutoff time (28px CondensedBold) + human reason ("After this, caffeine competes with your sleep.") + horizontal timing bar. One data point, one clear visualization.
10. **Dark card slider gradients are contrast-adjusted.** Low-end sweetness/acidity gradients start brighter on dark cards (#C8BFB0, #C8CCA0) for ~3.2:1 contrast against #3A3530.
11. **Subscribe & Save CTA flips priority after 3+ brews.** Default: Add to Cart primary, Subscribe secondary. After 3+ brews: Subscribe promoted to primary, Add to Cart demoted. The app nudges subscription when loyalty is demonstrated.
12. **Subscription management lives in Settings.** "Manage Subscriptions" row deep-links to Shopify's subscription management page via SFSafariViewController.
13. **Regent has 4 tasting notes.** Cocoa, Jasmine, Strawberry, Lime. Was missing Lime in R1.
14. **Stitch shows emotional tag on card.** `showEmotionalTagOnCard: true` — "For when you mean business. Dense, structured, no apologies." surfaces on the card in selection and marketplace contexts. Other beans show emotional tags only in detail view.
15. **Card 3 bridge copy reads the body, not the coffee.** "Steady morning. Standard brew." (First Light) connects health state to action without previewing the bean. The bean reveal is Screen 3's job.
16. **Marketplace subtitle connects both halves.** "Selected for cold brew. Matched to your morning." — health and coffee in one line.
17. **Empty filter state guides to nearby profile.** "Nothing matched Deep Work yet. Try Voltage — similar energy, more options."
18. **Subscribe banner CTA is actionable.** "SUBSCRIBE ON ANY BEAN" scrolls to first subscribable bean, not a generic landing page.
19. **Why Today animates once per day.** First visit: full typewriter + palette crossfade (~4s). Return visits same day: instant render, background starts warm.
20. **Navigation stays single push stack.** No tab bar. Marketplace is a pushed view. The dark/warm palette split would fight a persistent tab bar.

---

## 18. Copy Audit — All New Strings (Round 2)
...
"Clean start. These match the morning."
...
### Bean History (4 template strings)
```swift
...
**Total new strings: ~85** (including template variants)
...
**Phase 1 (Days 1-2):** Dual palette tokens + `BeanCardView.swift` + `FlavorSliderView.swift` + `TastingNoteView.swift`. Get a single bean card rendering with real data in all three context modes. This is the visual proof-of-concept.

**Phase 2 (Days 3-4):** `WhyTodayView.swift` (Screen 2) + `BeanSelectionView.swift` (Screen 3) + `GeneralProfileCard.swift`. Wire the emotional bridge and bean-first flow. Dashboard Card 3 CTA changes. Typewriter animation + palette crossfade.

**Phase 3 (Day 5):** `BrewSettingsView.swift` (Screen 4) + `ServingInfoCard.swift` + `TimingBarView.swift`. Brew parameters with bean-specific adjustments. Serving infographic with cup icons and timing bar.

**Phase 4 (Day 6):** `BeanDetailView.swift` + `BeanCardCompactView.swift`. Full detail screen with origin, flavor, brew pairing. Compact horizontal scroll variant.

**Phase 5 (Day 7):** `MarketplaceView.swift` + `FilterChipView.swift`. Bean Shop with profile filtering, subscribe banner, guided empty states. Wire all access points (dashboard, nav cart, settings).
...
**Phase 7 (Day 9):** Polish. Animations (Why Today typewriter, card staggers, slider springs). Dark/light card variant edge cases. Navigation flow testing. Post-brew bean name line. Subscribe & Save Shopify deep-links.

**Total: ~9 days.** Phase 2 gained a day for Why Today (new screen with animation complexity). Phase 6 is new (bean history feature). Can be parallelized — Phase 1 components are independent of Phase 2 flow work.
...
*Round 1 built the machine. Round 2 gave it a soul. Why Today is the breath between your body and your coffee. The bean history is the memory that makes you come back. The Corvus fix is the truth. Everything else got sharper — the cards split by intent, the general profile earned its place, and the clock arc died so the timing bar could live.*

*The health side reads your body. The coffee side reads like a menu at your favorite roaster. Why Today is the moment where they shake hands.*
