# Brezi iOS App — Iteration 4, Round 3 Design Spec
**Author:** Picasso (Design) | March 20, 2026
**Status:** Final spec for engineering handoff — all Round 2 feedback resolved
**Architecture:** Locked Iteration 4 — wellness-first, simplified daily flow

---

## Round 3 Changelog (from Round 2)

| # | Change | Source |
|---|---|---|
| 1 | Brief corrected: `brew_start + 25h` → `brew_end + 15h (~3 half-lives to ~12.5%)` | Elon R2 Fix 1 |
| 2 | Brief corrected: check-in pill "➖ Too light" → "➖ Too much" | Elon R2 Fix 2 |
| 3 | Manual recovery: daily reset, disabled START BREW gate, same-day read-only return, new UserDefaults keys | Elon R2 Fix 3 + Picasso tightening |
| 4 | History progressive states: explicit clarification that at 5+ check-ins, insight line is not rendered — pattern card takes its position | Elon R2 question, Picasso clarification |

## Cumulative Changelog (all rounds)

| Round | # | Change |
|---|---|---|
| R2-1 | 1 | Sleep-safe calculation: `brew_end + 10h` → `brew_end + 15h` (~12.5% remaining) |
| R2-2 | 2 | Long-press dose acceleration on [+]/[-] (hold > 0.5s, 5g per 0.2s) |
| R2-3 | 3 | Boundary glow on dose at min (30g) and max (120g) |
| R2-4 | 4 | History insight line at 3 check-ins: "3 brews logged. Patterns emerging." |
| R2-5 | 5 | One-time toast after 3rd check-in: "Noted. A few more and I'll start seeing patterns." |
| R2-6 | 6 | "BREW ANYWAY" subtext shows projected sleep-safe time |
| R2-7 | 7 | Kill stale `brew_start + 25h` reference — sleep-safe is `brew_end + 15h` everywhere |
| R3-1 | 8 | Brief corrected: caffeine timer `brew_end + 15h (~3 half-lives to ~12.5%)` |
| R3-2 | 9 | Brief corrected: check-in pill "➖ Too much" (was "Too light") |
| R3-3 | 10 | Manual recovery: daily reset + disabled START BREW gate + same-day read-only return |
| R3-4 | 11 | History: at 5+ non-skip check-ins, insight line is not rendered; pattern card takes its position |
...
3. Manual input → recovery chips + slider (see manual input spec below)

---

#### ⟐ R3 — Manual Recovery Input (no wearable connected)

**Layout — New Day (no selection yet):**
...
│                                      │
│  ┌──────┐  ┌────────────┐  ┌───────┐│
│  │🟢 High│  │🟡 Functional│  │🔴 Rest ││  ← 3 chips, no selection
│  └──────┘  └────────────┘  └───────┘│
│                                      │
│                                      │  ← slider HIDDEN until chip tapped
│                                      │
└──────────────────────────────────────┘
```

**Layout — After chip tapped (slider revealed):**
...
│                                      │
│  ┌──────┐  ┌────────────┐  ┌───────┐│
│  │🟢 High│  │🟡 Selected  │  │🔴 Rest ││  ← selected chip highlighted
│  └──────┘  └────────────┘  └───────┘│
│                                      │
│  Or slide:  ○──────●──────○   6     │  ← slider, 1–10, anchored to chip
│                                      │
└──────────────────────────────────────┘
```

**Layout — Same-day return (already logged today):**

```
┌──────────────────────────────────────┐
│  RECOVERY                            │
│                                      │
│           60%                        │  ← t_display (48px), white, centered
│                                      │
│  🟡 Functional · Logged today        │  ← t_mono (13px), #888
│                                      │
│  ● ● ● ● ● ● ○ ○ ○ ○               │  ← recovery dots
│                                      │
│  Update                              │  ← t_small (14px), #488BED, underline
│                                      │
└──────────────────────────────────────┘
```
...
| Text | `t_small` (14px), `#FFFFFF` |
| Selection haptic | `UIImpactFeedbackGenerator(.light)` |

**Manual chip → value mapping:**
...
| Property | Value |
|---|---|
| Visibility | Hidden until a chip is tapped. Fades in with `.easeOut(duration: 0.25)` |
| Range | 1–10, integer steps |
| Track bg | `#333333` |
| Track fill | `#488BED` |
| Thumb | 20pt circle, `#FFFFFF` |
| Value label | Slider value (1–10) — `t_bodyBold` (16px), `#FFFFFF`, right of slider |
| Label | "Or slide:" — `t_caption` (13px), `#888888`, left-aligned |
| Slider → recovery mapping | `slider_value × 10` (1 = 10%, 10 = 100%) |
| Haptic | `UISelectionFeedbackGenerator` on each step |

**⟐ R3 — Daily reset logic:**
...
**⟐ R3 — Same-day return behavior:**

| Property | Value |
|---|---|
| Condition | `Calendar.current.isDate(Date(), inSameDayAs: lastRecoveryInputDate) == true` |
| Display | Recovery card shows percentage (from `lastRecoveryValue × 10`), chip name, "Logged today" label, recovery dots |
| "Update" link | `t_small` (14px), `#488BED`, underline, centered below chip label |
| Update action | Tapping "Update" transitions card back to chip + slider selection mode (pre-filled with current values). New selection overwrites `lastRecoveryValue`, `lastRecoveryChip`, `lastRecoveryInputDate`. |
| START BREW | Enabled (recovery already logged) |

**Why the slider is hidden until chip tapped:** A bare slider with no anchor invites "leave it in the middle." Hiding the slider forces one conscious choice first — the chip sets the range, the slider refines. Two steps, but the first is a 1-second tap. This matches the intentionality of wearable data: WHOOP doesn't let you skip your recovery score, and neither do we.
...
**Long-press dose acceleration (R2):**
...
| Scale animation | Dose number pulses at 1.08 per tick — subtler for continuous gesture |
...
- Fires on both single-tap and long-press when hitting the boundary
...
**⟐ R3 — Disabled state (manual input, no recovery logged today):**

| Property | Value |
|---|---|
| Condition | `inputMode == "manual"` AND `lastRecoveryInputDate` is not today |
| Button background | `#333333` |
| Button text | "START BREW" — `Font.system(size: 17, weight: .bold)`, `#666666` |
| Tap response | None (disabled) |
| Gate copy | "Log your recovery to unlock today's brew." — `t_caption` (13px), `#888888`, centered, 8pt below button |

**Note:** When a wearable is connected (`inputMode == "whoop"` or `"oura"`), START BREW is always enabled — recovery data arrives automatically via API. The disabled gate only applies to the manual input path.

**Past-window state with projected sleep-safe time (R2):**

If current time > cutoff AND button is enabled, button and subtext change:
...
| `estimated_brew_duration` | Use current profile's recommended steep time (default 12 min) |

Example at 3:30 PM with First Light profile: "Past your window. Sleep-safe by 6:42 AM." — The data does the discouraging. User keeps agency.
...
│ │  🌙 Sleep-safe by 3:14 AM            │ │  ← tertiary, static: brew_end + 15h
│ │                                      │ │
│ │  ┌──────────────────────────────┐    │ │
│ │  │▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓░░░░░░░░░░░░ │    │ │  ← caffeine decay bar
│ │  │ ~120mg              → 0mg   │    │ │
...
#### Caffeine Time Calculations (FINAL — R2 corrected, R3 confirmed)
...
**⟐ R3 — Confirmed copy:** The triad is "Right" / "Needed more" / "Too much". This forms a clean signal triangle: correct / under / over. The brief has been corrected to match (was "Too light").
...
- Non-skip ratings increment `nonSkipCheckInCount` in `UserDefaults`

**One-time toast after 3rd check-in (R2):**

After the user's 3rd non-skip check-in rating, show a toast:
...
| Trigger | Check after each non-skip check-in: `if nonSkipCheckInCount == 3 && !hasShownPatternTeaser` |
...
6. ⟐ R3: Set `lastRecoveryInputDate = Date()`, `lastRecoveryValue` = mapped slider value (8/6/3), `lastRecoveryChip` = selected chip key. This ensures the manual recovery gate is satisfied on first launch.
...
│  History                                 │  ← t_h1, white
│                                          │
│  📊 3 brews logged. Patterns emerging.   │  ← insight line (3–4 check-ins)
│                                          │
│ ┌──────────────────────────────────────┐ │
...
#### ⟐ R3 — Progressive States (clarified)

| Non-skip check-ins | What shows | Notes |
|---|---|---|
| 0 brews | Empty state copy (centered) | "No brews yet. Your pattern shows up after the first few mornings." |
| 1–2 | Brew log entries only | No insight line (too early to promise anything) |
| 3–4 | Insight line + brew log entries | "📊 {count} brews logged. Patterns emerging." |
| 5+ | Pattern card + brew log entries | **Insight line is NOT rendered.** Pattern card takes its position directly. No transition — the pattern card is the payoff. |

**Insight line spec (R2):**
...
| Appears after | 5+ brews with non-skip check-in data |
...
| `recovery_logged_today` | "Logged today" |
...
| `brew_cta` | "START BREW" |
| `brew_cta_disabled_gate` | "Log your recovery to unlock today's brew." |
...
### Dose Long-Press Acceleration (R2)
...
### Dose Boundary Glow (R2)

```swift
// Trigger: dose reaches 30g (min) or 120g (max)
// Fires on both single-tap and long-press
if dose == minDose || dose == maxDose {
...
### ⟐ R3 — Manual Recovery Slider Reveal

```swift
// Trigger: chip tapped on manual recovery card (new day, no prior selection)
...
| `lastRecoveryInputDate` | Date? | nil | ⟐ R3: Gate daily manual recovery reset |
| `lastRecoveryValue` | Int? | nil | ⟐ R3: Last submitted recovery slider value (1–10), for same-day read-only display |
| `lastRecoveryChip` | String? | nil | ⟐ R3: "high" / "functional" / "rest" — for same-day read-only display |
...
### Caffeine Time Calculations (FINAL)
...
func caffeineTimestamps(brewEnd: Date) -> (peak: Date, halfLife: Date, sleepSafe: Date) {
    ...
    let sleepSafe  = brewEnd.addingTimeInterval(15 * 3600)    // +15h
...
AppRoot
 ...
 └── TabView (4 tabs, persistent post-onboarding)
      ...
      │    │    └── ⟐ R3: Manual recovery gate — START BREW disabled until logged
      ...
      │    ├── Insight line (3–4 non-skip check-ins)
      │    ├── Pattern card (5+ non-skip check-ins — replaces insight line)
      │    └── Brew log entries
      │
...
**Hard rule validation:** Open → Home (1 tap) → START BREW (1 tap) = **2 taps from open to START BREW**. Under the 3-tap maximum. (For manual input users on a new day: Open → tap chip (1 tap) → START BREW (1 tap) = **2 taps**. The chip tap is the recovery log — it's part of the daily practice, not friction.)
...
| Manual recovery resets daily — no stale data | Calendar day reset + disabled START BREW gate | ✓ R3 |
| Check-in signal triad is correct/under/over | "Right" / "Needed more" / "Too much" | ✓ R3 |
| Brief matches spec on all formulas | `brew_end + 15h`, "Too much" — both corrected | ✓ R3 |

---

## 13. Brief Corrections (R3)

The following corrections must be applied to the locked brief to maintain source-of-truth consistency:

### Correction 1: Caffeine Countdown Timer Card

**Brief currently reads:**
> - "🌙 Sleep-safe by 7:00 PM" → static (brew_start + 25h, i.e. ~5 half-lives to <3%)

**Must be updated to:**
> - "🌙 Sleep-safe by 10:12 PM" → static (brew_end + 15h, i.e. ~3 half-lives to ~12.5%)

### Correction 2: Check-In Card
...
**Why "Too much" not "Too light":** The check-in triad must form a clean signal triangle: correct / under / over. "Too light" paired with ➖ and sitting next to "Needed more" creates two options meaning roughly the same thing. "Too much" completes the triangle. More critically: if the pattern engine reads "too_light" as meaning "dose was excessive" (matching the ➖ emoji semantics), every generated pattern will have inverted recommendations. This is a data integrity issue, not a copy preference.

---

## 14. Round 3 Changes — Inline Reference

Every Round 3 change is marked inline with **⟐ R3** or **⟐ ROUND 3** wherever it appears in this spec. Search for these markers to find all changes from Round 2.

**Changes summary (4 total):**

1. **Section 13 / Brief Corrections** — Caffeine timer formula corrected in brief: `brew_end + 15h`
2. **Section 13 / Brief Corrections** — Check-in pill corrected in brief: "➖ Too much"
3. **Section 2A / Recovery Card** — Manual recovery daily reset: calendar day check, slider hidden until chip tapped, disabled START BREW gate with copy, same-day read-only return with "Update" link. New UserDefaults keys: `lastRecoveryInputDate`, `lastRecoveryValue`, `lastRecoveryChip`.
4. **Section 5 / History** — Explicit clarification: at 5+ non-skip check-ins, insight line is not rendered. Pattern card takes its position directly.

**What didn't change (and why):**
- Sleep-safe formula: resolved in R2, confirmed in R3. No changes.
- Long-press dose acceleration: resolved in R2. No changes.
- Boundary glow: resolved in R2. No changes.
- "BREW ANYWAY" with projected sleep-safe: resolved in R2. No changes.
- Insight line at 3–4 check-ins: resolved in R2. R3 only clarified the 5+ transition.
- One-time toast after 3rd check-in: resolved in R2. No changes.
- All onboarding screens: locked since R1. Only addition is setting `lastRecoveryInputDate` on first recovery input.
- Beans tab: locked since R1. No changes.
- Tab bar: locked since R1. No changes.
- Evening state: stays minimal. Restraint is the brand.
- All copy (except brief corrections and new gate copy): locked since R2.
- All animations (except manual recovery slider reveal): locked since R2.

**Deferred to v1.1:**
- Recovery dots vs loading indicator ambiguity (low risk — 48px percentage above removes ambiguity)
- Evening state with real-time wearable data (no reliable data source yet)
- Hardware mention in onboarding (marketing concern, not design)

---

*Round 3 spec written March 20, 2026. All Round 2 feedback resolved. Two brief corrections applied. Manual recovery daily reset fully specified. Architecture locked. Copy locked. Ship it.*